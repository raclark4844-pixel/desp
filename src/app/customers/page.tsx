...
export default async function CustomersPage({ searchParams }: { searchParams: Promise<{ q?: string; page?: string }> }) {
  ...
  return <main className="page-shell">
    ...
    {customers.length ? <div className="customer-table-scroll"><table className="customer-table"><caption>Saved customer information</caption><thead><tr><th scope="col">Company</th><th scope="col">Primary contact</th><th scope="col">Email</th><th scope="col">Website</th><th scope="col">Time zone</th><th scope="col">Status</th><th scope="col">Campaigns</th></tr></thead><tbody>
      {customers.map(customer => <tr key={customer.id}><th scope="row">{customer.name}<small>Customer ID: {customer.id}</small></th><td>{customer.contactName || "Not saved"}</td><td>{customer.contactEmail || "Not saved"}</td><td>{customer.websiteUrl && /^https?:\/\//i.test(customer.websiteUrl) ? <a href={customer.websiteUrl} target="_blank" rel="noopener noreferrer">{customer.websiteUrl}</a> : "Not saved"}</td><td>{customer.timezone}</td><td>{customer.status.toLowerCase()}</td><td>{customer._count.campaigns}</td></tr>)}
